...
int FP6_mul(FP6 *r, FP6 *a, FP6 *b) {
	...
	/* v0 = a_0b_0 */
	if (!FP2_mul(&group, &v0, &a->f[0], &b->f[0], group.bn)) {
		goto err;
	}

	/* v1 = a_1b_1 */
	if (!FP2_mul(&group, &v1, &a->f[1], &b->f[1], group.bn)) {
		goto err;
	}

	/* v2 = a_2b_2 */
	if (!FP2_mul(&group, &v2, &a->f[2], &b->f[2], group.bn)) {
		goto err;
	}
	...
	if (!FP2_mul(&group, &t2, &t0, &t1, group.bn)) {
		goto err;
	}
	...
	if (!FP2_mul_nor(&group, &t0, &t2, group.bn)) {
		goto err;
	}
	...
	if (!FP2_mul(&group, &r->f[1], &t0, &t1, group.bn)) {
		goto err;
	}
	...
	if (!FP2_mul_nor(&group, &t0, &v2, group.bn)) {
		goto err;
	}
	...
	if (!FP2_mul(&group, &r->f[2], &t0, &t1, group.bn)) {
		goto err;
	}
...
int FP6_mul_unr(FP6 *r, FP6 *a, FP6 *b) {
	...
	/* v0 = a_0b_0 */
	if (!FP2_mul_unr(&group, &v0, &a->f[0], &b->f[0], group.bn)) {
		goto err;
	}

	/* v1 = a_1b_1 */
	if (!FP2_mul_unr(&group, &v1, &a->f[1], &b->f[1], group.bn)) {
		goto err;
	}

	/* v2 = a_2b_2 */
	if (!FP2_mul_unr(&group, &v2, &a->f[2], &b->f[2], group.bn)) {
		goto err;
	}
	...
	if (!FP2_mul_unr(&group, &t2, &t0, &t1, group.bn)) {
		goto err;
	}
	if (!FP2_sub(&group, &t2, &t2, &v1)) {
		goto err;
	...
	if (!FP2_mul_unr(&group, &r->f[1], &t0, &t1, group.bn)) {
		goto err;
	}
	if (!FP2_sub(&group, &r->f[1], &r->f[1], &v0)) {
		goto err;
	...
	if (!FP2_mul_unr(&group, &r->f[2], &t0, &t1, group.bn)) {
		goto err;
	}
	if (!FP2_sub(&group, &r->f[2], &r->f[2], &v0)) {
		goto err;
...
int FP6_mul_dxs(FP6 *r, FP6 *a, FP6 *b) {
	...
	if (!FP2_mul(&group, &t0, &t0, &b->f[1], group.bn)) {
		goto err;
	}
	if (!FP2_sub(&group, &t0, &t0, &v1)) {
		goto err;
	}
	if (!FP2_mul_nor(&group, &t2, &t0, group.bn)) {
		goto err;
	}
	...
	if (!FP2_mul(&group, &r->f[2], &t0, &b->f[0], group.bn)) {
		goto err;
	}
	if (!FP2_sub(&group, &r->f[2], &r->f[2], &v0)) {
		goto err;
...
int FP6_rdc(FP6 *r, FP6 *a) {
	...
	/* c_0 = t1 mod p. */
	if (!FP2_rdc(&group, &r->f[0], &a->f[0], group.bn)) {
		goto err;
	}
	if (!FP2_rdc(&group, &r->f[1], &a->f[1], group.bn)) {
		goto err;
	}
	if (!FP2_rdc(&group, &r->f[2], &a->f[2], group.bn)) {
		goto err;
	}
...
int FP6_mul_art(FP6 *r, FP6 *a) {
	...
	FP2_copy(&t0, &a->f[0]);
	if (!FP2_mul_nor(&group, &r->f[0], &a->f[2], group.bn)) {
		goto err;
	}
...
int FP6_sqr(FP6 *r, FP6 *a) {
	...
	/* t0 = a_0^2 */
	if (!FP2_sqr(&group, &t0, &a->f[0], group.bn)) {
		goto err;
	}

	/* t1 = 2 * a_1 * a_2 */
	if (!FP2_mul(&group, &t1, &a->f[1], &a->f[2], group.bn)) {
		goto err;
	}
	...
	/* t2 = a_2^2. */
	if (!FP2_sqr(&group, &t2, &a->f[2], group.bn)) {
		goto err;
	}
	...
	if (!FP2_sqr(&group, &r->f[2], &r->f[2], group.bn)) {
		goto err;
	}
	...
	/* c1 = t3 + t2 * E. */
	if (!FP2_mul_nor(&group, &t4, &t2, group.bn)) {
		goto err;
	}
...
int FP6_sqr2(FP6 *r, FP6 *a) {
	...
	/* t1 = 2 * a_0 * a_1 */
	if (!FP2_mul(&group, &t1, &a->f[0], &a->f[1], group.bn)) {
		goto err;
	}
	if (!FP2_add(&group, &t1, &t1, &t1)) {
		goto err;
	...
	if (!FP2_sqr(&group, &t2, &t2, group.bn)) {
		goto err;
	}

	/* t3 = 2 * a_1 * a_2 */
	if (!FP2_mul(&group, &t3, &a->f[1], &a->f[2], group.bn)) {
		goto err;
	}
	...
	/* t4 = a_2^2 */
	if (!FP2_sqr(&group, &t4, &a->f[2], group.bn)) {
		goto err;
	}

	/* c_0 = t0 + E * t3 */
	if (!FP2_mul_nor(&group, &r->f[0], &t3, group.bn)) {
		goto err;
	}
	...
	/* c_1 = t1 + E * t4 */
	if (!FP2_mul_nor(&group, &r->f[1], &t4, group.bn)) {
		goto err;
	}
...
int FP6_inv(FP6 *r, FP6 *a) {
	...
	if (!FP2_mul(&group, &v0, &a->f[1], &a->f[2], group.bn)) {
		goto err;
	}
	if (!FP2_mul_nor(&group, &v2, &v0, group.bn)) {
		goto err;
	}
	...
	/* v1 = E * a_2^2 - a_0 * a_1. */
	if (!FP2_sqr(&group, &t0, &a->f[2], group.bn)) {
		goto err;
	}
	if (!FP2_mul_nor(&group, &v2, &t0, group.bn)) {
		goto err;
	}
	if (!FP2_mul(&group, &v1, &a->f[0], &a->f[1], group.bn)) {
		goto err;
	}
	...
	/* v2 = a_1^2 - a_0 * a_2. */
	if (!FP2_sqr(&group, &t0, &a->f[1], group.bn)) {
		goto err;
	}
	if (!FP2_mul(&group, &v2, &a->f[0], &a->f[2], group.bn)) {
		goto err;
	}
	...
	if (!FP2_mul(&group, &t0, &a->f[1], &v2, group.bn)) {
		goto err;
	}
	if (!FP2_mul_nor(&group, &r->f[1], &t0, group.bn)) {
		goto err;
	}

	if (!FP2_mul(&group, &r->f[0], &a->f[0], &v0, group.bn)) {
		goto err;
	}

	if (!FP2_mul(&group, &t0, &a->f[2], &v1, group.bn)) {
		goto err;
	}
	if (!FP2_mul_nor(&group, &r->f[2], &t0, group.bn)) {
		goto err;
	}
	...
	if (!FP2_inv(&group, &t0, &t0, group.bn)) {
		goto err;
	}

	if (!FP2_mul(&group, &r->f[0], &v0, &t0, group.bn)) {
		goto err;
	}
	if (!FP2_mul(&group, &r->f[1], &v1, &t0, group.bn)) {
		goto err;
	}
	if (!FP2_mul(&group, &r->f[2], &v2, &t0, group.bn)) {
		goto err;
	}
